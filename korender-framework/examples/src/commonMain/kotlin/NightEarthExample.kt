...
private data class City(val name: String, val lat: Float, val lon: Float) {
    ...
    private fun latLonToVec3(latDeg: Float, lonDeg: Float): Vec3 {
        ...
        val theta = FloatMath.PI / 2f - lat
        return Vec3(sin(theta) * cos(lon), cos(theta), sin(theta) * sin(lon))
    }
}
...
fun NightEarth() {
    ...
    Korender(resourceLoader = { Res.readBytes("files/$it") }) {
        Frame {
            ...
            val up = (1.y - look * look.y).let { if (it.lengthSquared() > 1e-4f) it else 1.x }.normalize()
...
private fun GuiContainerScope.cityLink(city: City?, selected: (City?) -> Unit) {
    val text = city?.name ?: "> Flyover >"
    Text(
        id = text,
        text = text,
        style = TextStyle(height = 24f, color = ColorRGBA(0xF06543A0)),
        onTouch = {
