...
class CollisionDetector {
    ...
    private val handlers = mutableMapOf<KClass<out Any>, MutableMap<KClass<out Any>, Handler<Any, Any>>>()
    ...
    fun go() {
        ...
        for (i in 0..<xlist.size - 1) {
            val t = xlist[i]
            val o = xlist[i + 1]
            if (abs(t.position.x - o.position.x) < t.radius + o.radius) {
        ...
        for (cp in candidatePairs) {
            if ((cp.first.position - cp.second.position).lengthSquared() < (cp.first.radius + cp.second.radius) * (cp.first.radius + cp.second.radius)) {
