...
class GlGpuShader(
    ...
    private fun bindAttrs(mesh: GlGpuMesh) {
        ...
        for (attr in mesh.attrs) {
            val location =
                requireNotNull(attributeLocations[attr.name]) { "Mesh attr ${attr.name} not found in shader" }
            VGL20.glEnableVertexAttribArray(location)
            VGL20.glVertexAttribPointer(location, attr.size, VGL11.GL_FLOAT, false, mesh.vertexSize, offset)
