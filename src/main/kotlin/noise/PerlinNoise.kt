...
import com.zakgof.korender.noise.Noise2
import java.util.*
import kotlin.math.floor

class PerlinNoise (private val seed: Float = (Random().nextGaussian() * 255f).toFloat()) : Noise2 {

    private var default_size: Long = 35L
    private var p: IntArray = IntArray(512)
    private var permutation = intArrayOf(
    ...
    fun noise(x: Float, y: Float, z: Float, initSize: Int): Float {
        var size = initSize
        var value = 0f
        val initialSize = size.toFloat()
    ...
    fun smoothNoise(initx: Float, inity: Float, initz: Float): Float {
