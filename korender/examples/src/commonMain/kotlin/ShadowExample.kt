...
fun ShadowExample() {
    Korender() {
        ...
        val plate = Meshes.cube(1f) {
            transformPos(
                Transform().scale(8f, 1f, 5f).translate(-1.6f.y)
            ) // TODO: vertex transformation
        }.build(gpu)
        val material = Materials.standard(gpu, "SHADOW_RECEIVER", "PCSS") {
            colorFile = "/sand.jpg"
        }

        val rcube = Renderables.create(Meshes.cube(1.5f).build(gpu), material)
        val rsphere = Renderables.create(Meshes.sphere(1.5f).build(gpu), material)
        val rplate = Renderables.create(plate, material)
        ...
        onFrame = { frameInfo ->
            rcube.transform =
                Transform().rotate(1.x, -FloatMath.PIdiv2).rotate(1.y, frameInfo.time * 0.1f)
            rsphere.transform =
                Transform().translate(Vec3(-4.0f, 2.0f + sin(frameInfo.time), 0.0f));
        }
