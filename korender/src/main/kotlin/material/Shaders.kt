...
object Shaders {
    fun create(gpu: Gpu, vertexShaderFile: String, fragmentShaderFile: String, vararg defs: String): GpuShader =
...
private class ShaderBuilder(vertexShaderFile: String, fragmentShaderFile: String, vararg defs: String) {
    ...
    init {
        val vfsplit: Array<String> = vertexShaderFile.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        ...
        this.defs = defs.toSet()
        // this.defs.add(VGL11.platform.shaderEnv()) // TODO: ugly
    ...
    private fun preprocessFile(startDir: String, fname: String, defs: Set<String>, debugInfo: ShaderDebugInfo): String {
        ...
        Scanner(code).use { scanner ->
            while (scanner.hasNextLine()) {
                ...
                if (line.trim { it <= ' ' } == "#else") {
                    ...
                    passes.pop()
                    if (inverting.startsWith("!") || inverting.startsWith("+")) throw java.lang.RuntimeException("Shader preprocessor error: repeated #else for $inverting")
