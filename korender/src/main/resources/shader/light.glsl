...
float shadow(sampler2D shadowTexture, vec3 vshadow) {
    #ifdef PCSS
        float centerOccluderDepth = texture2D(shadowTexture, vshadow.xy).r;
        ...
        for (int x = -2; x <= 2; ++x) {
            for (int y = -2; y <= 2; ++y) {
                ...
                float shadowSample = texture2D(shadowTexture, vshadow.xy + vec2(x, y) * penumbraWidth).r;
