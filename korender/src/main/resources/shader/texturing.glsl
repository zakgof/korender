...
{   
   vec4 samp = _texture2D(noisetex, pos * sz);
...
vec4 wang(sampler2D wangIndex, sampler2D tileset, vec2 c, float rp)	
{				
	vec2 tileCoords = _texture2D(wangIndex, vec2(c.x, c.y)).rg;
	tileCoords = vec2(tileCoords.x * 5.0 * 0.25, 0.75 - tileCoords.y * 5.0 * 0.25);	
	vec2 inTile = vec2(mod(c.x, 1.0/rp) * rp * 0.25, mod(c.y, 1.0/rp) * rp * 0.25);	
	vec2 w = inTile + tileCoords;
	
	#ifndef GL_ES
		vec2 through = c * rp * 0.25;
		vec2 dPdx = dFdx(through);
		vec2 dPdy = dFdy(through);	
		return textureGrad(tileset, w, dPdx, dPdy);
	#else			
		return _texture2D(tileset, w);
	#endif
